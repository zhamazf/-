 ...
 *  blockedUsers/{uid}/{blockedUid}        → { blockedAt, reason }  ← v2: مُوحَّد مع auth.js
 *  notifications/{uid}/{notifId}          → { type, fromUid, ts, read }
 * ────────────────────────────────────────────────────────────
 *
 * v2 fixes:
 * 1. مسار الحظر موحَّد: blocked/ → blockedUsers/ (يتطابق مع auth.js)
 * 2. acceptRequest → يُرسل إشعاراً للطرف الآخر
 * 3. sendRequest   → يُرسل إشعاراً للمستهدف
 * 4. notifyUser()  → دالة مساعدة مركزية للإشعارات
 */
window.Friends = (function () {

  /* ── Constants ──────────────────────────────────────────── */
  const MAX_FRIENDS       = 500;  // حد أقصى للأصدقاء
  const REQUEST_COOLDOWN  = 30000; // 30 ثانية بين كل طلب صداقة

  // rate limit محلي لطلبات الصداقة
  const _lastRequestTime = new Map(); // targetUid → timestamp
  ...
  /* ═══════════════════════════════════════════════════════════
     NOTIFY USER (v2) — دالة مساعدة مركزية للإشعارات
     Path: notifications/{targetUid}/{notifId}
     type: 'friend_request' | 'friend_accept'
  ═══════════════════════════════════════════════════════════ */
  async function notifyUser(targetUid, type, fromUid, extra = {}) {
  ...
  async function sendRequest(meUid, targetUid, users, myFriends) {
    ...
    if (myFriends.length >= MAX_FRIENDS)
      return { error: `وصلت للحد الأقصى (${MAX_FRIENDS} صديق)` };

    const lastReq = _lastRequestTime.get(targetUid) || 0;
    if (Date.now() - lastReq < REQUEST_COOLDOWN) {
      const secs = Math.ceil((REQUEST_COOLDOWN - (Date.now() - lastReq)) / 1000);
      return { error: `انتظر ${secs} ثانية قبل إعادة الإرسال` };
    }

    // v2: مسار الحظر موحَّد مع auth.js → blockedUsers/
    const [blockedByMe, blockedByThem] = await Promise.all([
      FB.get(`blockedUsers/${meUid}/${targetUid}`),
      FB.get(`blockedUsers/${targetUid}/${meUid}`),
    ]);
    ...
    _lastRequestTime.set(targetUid, Date.now());

    // v2: إشعار للمستهدف
    await notifyUser(targetUid, 'friend_request', meUid);
  ...
  /* ═══════════════════════════════════════════════════════════
     ACCEPT REQUEST
     ✅ atomic multi-path update + إشعار للطرف الآخر (v2)
  ═══════════════════════════════════════════════════════════ */
  async function acceptRequest(meUid, fromUid) {
    ...
    updates[`friendRequests/${meUid}/${fromUid}`] = null;
    await FB.upd('/', updates);

    // v2: إشعار للطرف الذي أرسل الطلب
    await notifyUser(fromUid, 'friend_accept', meUid);
  ...
  async function removeFriend(meUid, friendUid) {
    ...
    // atomic: يحذف الصداقة + أي طلبات معلقة في الاتجاهين
    const updates = {};
    updates[`users/${meUid}/friends/${friendUid}`]        = null;
    updates[`users/${friendUid}/friends/${meUid}`]        = null;
    updates[`friendRequests/${meUid}/${friendUid}`]       = null;
    updates[`friendRequests/${friendUid}/${meUid}`]       = null;
    await FB.upd('/', updates);
    return { ok: true };
  }
  ...
  /* ═══════════════════════════════════════════════════════════
     BLOCK / UNBLOCK (v2)
     مسار موحَّد: blockedUsers/ (يتطابق مع auth.js)
  ═══════════════════════════════════════════════════════════ */
  async function blockUser(meUid, targetUid) {
    if (!targetUid || targetUid === meUid) return { error: 'بيانات غير صحيحة' };

    // إذا كانوا أصدقاء → احذف الصداقة وأي طلبات معلقة
    const updates = {};
    updates[`blockedUsers/${meUid}/${targetUid}`]       = { blockedAt: Date.now() };
  ...
  async function unblockUser(meUid, targetUid) {
    ...
    const blocked = await FB.get(`blockedUsers/${meUid}/${targetUid}`);
    if (!blocked) return { error: 'هذا المستخدم غير محظور' };
    await FB.del(`blockedUsers/${meUid}/${targetUid}`);
    return { ok: true };
  }
  ...
  async function getBlockedList(meUid) {
    if (!meUid) return [];
    const data = await FB.get(`blockedUsers/${meUid}`);
   ...
   * @param {number}  limit
   */
  function searchUsers(query, users, meUid, myFriendUids = [], blockedUids = [], limit = 10) {
    if (!query || !query.trim()) return [];
    const q          = query.trim().toLowerCase();
    const excludeSet = new Set([meUid, ...blockedUids]);
    const friendSet  = new Set(myFriendUids);
    ...
    return Object.entries(users)
      ...
      .map(([uid, u]) => ({
        uid,
        ...u,
        isFriend: friendSet.has(uid) // مفيد للـ UI
      }));
  }
  ...
  return {
    ...
    // Block / unblock (v2: موحَّد مع auth.js)
    blockUser,
    unblockUser,
    isBlocked,
    getBlockedList,
