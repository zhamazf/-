...
window.Stories = (function () {
  ...
  function cleanExpired(stories, meUid) {
    // Guard: if no uid yet, skip entirely
    if (!meUid) return;
    // Only delete own stories (uid-keyed) — never touch other users' nodes
    const now = Date.now();
    const myStories = (stories || {})[meUid];
    if (!myStories) return;
    Object.entries(myStories).forEach(([sid, s]) => {
      if (s && now - s.ts > STORY_TTL) {
        FB.del(`stories/${meUid}/${sid}`);
      }
    });
  }
  ...
  function friendsWithStories(stories, me, myFriends) {
    // myFriends is array of usernames; stories may be keyed by uid or username
    // Accept both: match by direct key OR by value in myFriends
    return usersWithStories(stories, me).filter(u =>
      myFriends.includes(u) || u === me
    );
  }
