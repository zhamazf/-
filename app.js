...
(async function NabdaApp() {
  ...
  function go(screen, id=null) {
    ...
    if (screen==='dm'&&id&&S.meUid) { const tUid=_uidByName(id)||id; Chat.markRead(Chat.dmKey(S.meUid, tUid), S.meUid); }
  ...
  function startMsgListener(type, id) {
    ...
    const targetUid = _uidByName(id) || id;
    const path = type==='dm' ? `dmMsgs/${Chat.dmKey(S.meUid, targetUid)}` : `groupMsgs/${id}`;
    ...
    const cb = msgs => {
      try {
        ...
        if (type==='dm') { const tUid=_uidByName(id)||id; Chat.markRead(Chat.dmKey(S.meUid, tUid), S.meUid); }
      } catch (err) {
    ...
    try {
      const tUid6=_uidByName(id)||id; _msgUnsub = type==='dm' ? Chat.listenDM(S.meUid,tUid6,cb) : Chat.listenGroup(id,cb);
    } catch (err) {
  ...
  function startDataListeners() {
    addU(FB.on('users',    v=>{S.users=v||{}; rebuildUserMaps(); refreshDyn(); _startDmListeners(); _startFriendReqListener(); _startStoryListeners();}));
    ...
    setTimeout(() => {
      addU(FB.on(`stories/${S.meUid}`, v=>{
        // Read only own stories (Rules: $uid === auth.uid for write, .read: auth!==null is global)
        if (S.meUid) S.stories[S.meUid] = v || null;
        refreshDyn();
      }));
      // Also listen to friends' stories individually when friends list is known
      _startStoryListeners();
      if (!_storiesTimer) _storiesTimer = setInterval(()=>Stories.cleanExpired(S.stories, S.meUid), 300000);
    }, 2000);
  }

  // ── DM listeners: one per friend uid pair (Rules: $dmKey.contains(auth.uid)) ──
  const _dmUnsubs = {};
  ...
  // ── friendRequests listener: own uid only (Rules: $targetUid === auth.uid) ──
  ...
  // ── Stories: listen per-uid (own + each friend) ──
  const _storyUnsubs = {};
  ...
  function buildStoryInner(container){
    ...
    if(myS.length){const w=UI.el('div','display:flex;flex-direction:column;align-items:center;gap:3px;flex-shrink:0');const ring=UI.el('div','padding:2px;border-radius:50%;border:2.5px solid #22c55e;display:inline-flex;cursor:pointer');ring.appendChild(UI.makeAvatar(_userData(S.me),46));ring.addEventListener('click',()=>openStoryViewer(S.me));const lbl=UI.el('span','font-size:10px;color:#22c55e');lbl.textContent='قصتي';w.appendChild(ring);w.appendChild(lbl);container.appendChild(w);}
    fws().forEach(u=>{
  ...
  function renderTabContent(container){
    ...
    if(S.tab==='chats'){
      ...
      const others=Object.keys(S.usersByName).filter(u=>u!==S.me&&Chat.getDMMessages(S.me,u,S.dmChats).length>0);
  ...
  function buildChatItem(u,forcedUnread=0){
    ...
    const uid = _uidByName(u);
    item.appendChild(UI.makeAvatar(_userData(u),44,{online:Chat.isOnline(S.presence, uid || u),ring:hasSt,seen:seenSt,onClick:hasSt?e=>{e.stopPropagation();Stories.markSeen(u);openStoryViewer(u);}:null}));
  ...
  function _statusText(m) {
    if (m.sender !== S.meUid && m.sender !== S.me) return '';
  ...
  function sendMedia(payload,type){if(type==='dm'){const tUid=_uidByName(S.activeId)||S.activeId;Chat.sendDM(S.meUid,tUid,payload);}else{Chat.sendGroup(S.activeId,S.meUid,payload);}UI.playSound('send');}
  ...
  function buildGroupPanel(parentEl){
    ...
    if(!Groups.isAdmin(g,S.meUid)){
      const leaveWrap=UI.el('div',`padding:12px 14px;border-top:1px solid ${th().border}`);
      const leaveBtn=UI.el('button','width:100%;padding:9px;border:1px solid #fecaca;border-radius:10px;background:transparent;color:#ef4444;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer');
      leaveBtn.textContent='🚪 مغادرة المجموعة';
      ...
      leaveWrap.appendChild(leaveBtn);
      panel.appendChild(leaveWrap);
    }
  ...
  function renderUserProfile(){
    ...
    const uid = _uidByName(u);
    avWrap.appendChild(UI.makeAvatar(userData,100,{online:Chat.isOnline(S.presence, uid || u),ring:fws().includes(u),seen:Stories.isSeen(u,S.stories)}));
  ...
  function renderFriends(){
    ...
    const doSearch=async()=>{const target=sInp.value.trim();if(!target)return;const targetUid=_uidByName(target)||await Auth.uidFromUsername(target);if(!targetUid)return UI.toast('المستخدم غير موجود','error');const myFriendUids=Object.keys(S.users[S.meUid]?.friends||{});const res=await Friends.sendRequest(S.meUid,targetUid,S.users,myFriendUids);if(res.error)UI.toast(res.error,'error');else{UI.toast(`تم إرسال طلب صداقة إلى ${target} ✓`,'success');UI.playSound('notif');}};
  ...
  const res=await Stories.post(S.meUid,S.storyPreview);
  if(res&&res.error){UI.toast(res.error,'error');postBtn.disabled=false;postBtn.textContent='نشر القصة 📢';return;}
