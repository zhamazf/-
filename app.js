...
(async function NabdaApp() {
  ...
  // ── Groups listeners — نقرأ كل مجموعة يكون فيها المستخدم ──
  const _groupUnsubs = {};
  function _startGroupListeners() {
    if (!S.meUid) return;
    // أولاً: نجلب قائمة المجموعات من users/{meUid}/groups
    const myGroupIds = Object.keys(S.users[S.meUid]?.groups || {});
    myGroupIds.forEach(gid => {
      if (_groupUnsubs[gid]) return;
  ...
  function renderNewGroup(){
    ...
    const doCreate=async()=>{const res=await Groups.create(S.me,inp.value);if(res.error)UI.toast(res.error,'error');else{UI.toast('تم إنشاء المجموعة ✓','success');
      ...
      if (res.gid && !_groupUnsubs[res.gid]) {
        _groupUnsubs[res.gid] = FB.on(`groups/${res.gid}`, v => {
          if (v) S.groups[res.gid] = v; else delete S.groups[res.gid];
          refreshDyn();
        });
      }
