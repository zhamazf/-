...
(async function NabdaApp() {
  ...
  /* ═══════════════════════════════════════════════════════════
     MESSAGE LISTENER — append only, no duplicates
  ═══════════════════════════════════════════════════════════ */
  function startMsgListener(type, id) {
    ...
    const cb = msgs => {
      ...
      if (isFirst) {
        ...
        newMsgs.forEach(m=>{
          ...
          if (m.sender!==S.me) {
            UI.playSound('recv');
            UI.showNotif(type==='dm'?`رسالة من ${m.sender}`:`رسالة في مجموعة`, m.type==='text'?m.text:'🖼 وسائط', ()=>window.focus());
            appendMsg(m);
          } else {
            replaceOptimistic(m);
          }
    ...
    _msgUnsub = type==='dm' ? Chat.listenDM(S.me,id,cb) : Chat.listenGroup(id,cb);
  }

  function replaceOptimistic(serverMsg) {
    const c=document.getElementById('msg-container'); if(!c) return;
    const opt=c.querySelector('[data-opt]');
    if (opt) c.replaceChild(buildBubble(serverMsg), opt);
  }
  ...
  function refreshTopSub() {
    ...
    const tu=Chat.getTypingUsers(S.typingMap,S.me,S.activeId,S.screen);
    if (S.screen==='dm') { sub.textContent=tu.includes(S.activeId)?'✍️ يكتب...':Chat.lastSeenText(S.presence,S.activeId); sub.style.color=Chat.isOnline(S.presence,S.activeId)?'#22c55e':th().text3; }
    else if (S.screen==='group') { const g=ag(); if(!g) return; sub.textContent=`${Groups.getMembers(g).length} عضو${tu.length>0?' · ✍️ يكتب...':''}`; }
  }
  ...
  /* ═══════════════════════════════════════════════════════════
     AUTH SCREEN — Email + Password + Username
  ═══════════════════════════════════════════════════════════ */
  function renderAuth() {
    ...
    async function doSubmit() {
      ...
      S.me=res.username; S.meEmail=Auth.getEmail();
  ...
  function appendMsg(m){const c=document.getElementById('msg-container');if(!c)return;const emp=c.querySelector('.empty-chat');if(emp)emp.remove();c.appendChild(buildBubble(m));scrollBot();}
  ...
  function buildBubble(m, isOpt=false){
    ...
    if(isOpt)row.setAttribute('data-opt','1');
    ...
    const meta=UI.el('div',`font-size:10px;color:${th().text3};margin-top:2px;text-align:${mine?'left':'right'};padding-right:2px`); meta.textContent=UI.fmt(m.ts)+(mine?' ✓✓':''); col.appendChild(bub); col.appendChild(meta); row.appendChild(col); return row;
  }
  ...
  function buildChatInput(type){
    ...
    const doSend=()=>{
      ...
      const payload={type:'text',text}; const tempId='_opt_'+Date.now()+'_'+Math.random().toString(36).slice(2); const tempMsg={sender:S.me,ts:Date.now(),id:tempId,...payload};
      _seenMsgIds.add(tempId); appendMsg({...tempMsg,_isOpt:true});
      if(type==='dm')Chat.sendDM(S.me,S.activeId,payload); else Chat.sendGroup(S.activeId,S.me,payload);
      UI.playSound('send');
    };
  ...
  function buildMediaTray(tray,type){
    ...
    const imgIn=document.createElement('input');imgIn.type='file';imgIn.accept='image/*';imgIn.style.display='none';imgIn.addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;if(f.size>5*1024*1024)return UI.toast('الحجم الأقصى 5 ميغابايت','error');const d=await UI.fileToBase64(f);e.target.value='';sendMedia({type:'image',data:d},type);closeTray();});
    tray.appendChild(mkBtn('🖼','صورة',()=>imgIn.click())); tray.appendChild(imgIn);
    const vidIn=document.createElement('input');vidIn.type='file';vidIn.accept='video/*';vidIn.style.display='none';vidIn.addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;if(f.size>20*1024*1024)return UI.toast('الحجم الأقصى 20 ميغابايت','error');const d=await UI.fileToBase64(f);e.target.value='';sendMedia({type:'video',data:d},type);closeTray();});
  ...
  function buildGroupPanel(parentEl){
    ...
    agM.forEach(u=>{const isMod=g.mods&&g.mods[u];const isAdm=g.admin===u;const row=UI.el('div','display:flex;align-items:center;gap:8px;margin-bottom:10px');const avBtn=UI.el('button','background:none;border:none;cursor:pointer;padding:0');avBtn.appendChild(UI.makeAvatar(S.users[u],34,{online:Chat.isOnline(S.presence,u)}));avBtn.addEventListener('click',()=>go('userProfile',u));const info=UI.el('div','flex:1;min-width:0');const nmBtn=UI.el('button','background:none;border:none;cursor:pointer;padding:0;text-align:right;width:100%');const nmEl=UI.el('div',`font-size:12px;font-weight:${isAdm||isMod?700:400};color:${th().text};overflow:hidden;text-overflow:ellipsis;white-space:nowrap`);nmEl.textContent=u;const roleEl=UI.el('div',`font-size:9px;color:${isAdm?'#f59e0b':isMod?'#6366f1':Chat.isOnline(S.presence,u)?'#22c55e':th().text3}`);roleEl.textContent=isAdm?'👑 مشرف رئيسي':isMod?'🛡 مشرف مساعد':Chat.isOnline(S.presence,u)?'متصل':Chat.lastSeenText(S.presence,u);nmBtn.appendChild(nmEl);nmBtn.addEventListener('click',()=>go('userProfile',u));info.appendChild(nmBtn);info.appendChild(roleEl);row.appendChild(avBtn);row.appendChild(info);if(Groups.isAdmin(g,S.me)&&u!==S.me){const acts=UI.el('div','display:flex;gap:3px');const modBtn=UI.el('button','background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px');modBtn.textContent='🛡';modBtn.title=isMod?'إزالة المشرف':'ترقية لمشرف';modBtn.style.color=isMod?'#6366f1':th().text3;modBtn.addEventListener('click',()=>{Groups.toggleMod(g.id,u,!!isMod);UI.toast(isMod?'تمت إزالة المشرف':'أصبح مشرفاً مساعداً ✓','success');});const kickBtn=UI.el('button','background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px');kickBtn.textContent='🚫';kickBtn.style.color=th().text3;kickBtn.addEventListener('click',()=>Groups.kick(g.id,u));acts.appendChild(modBtn);acts.appendChild(kickBtn);row.appendChild(acts);}else if(Groups.isMod(g,S.me)&&!isAdm&&!isMod&&u!==S.me){const kickBtn=UI.el('button','background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px');kickBtn.textContent='🚫';kickBtn.style.color=th().text3;kickBtn.addEventListener('click',()=>Groups.kick(g.id,u));row.appendChild(kickBtn);}memSec.appendChild(row);});panel.appendChild(memSec);parentEl.appendChild(panel);
  }
  ...
  function renderProfile(){
    ...
    const emailEl=UI.el('div',`font-size:12px;color:${th().text3};margin-top:2px`);emailEl.textContent=S.meEmail||'';
    const dtEl=UI.el('div',`font-size:12px;color:${th().text3}`);dtEl.textContent=md.createdAt?`عضو منذ ${new Date(md.createdAt).toLocaleDateString('ar')}`:'';
    ...
    logBtn.addEventListener('click',async()=>{Chat.goOffline(S.me);await Auth.logout();S.me=null;S.meEmail=null;go('auth');});inner.appendChild(logBtn);
    // Delete account
    ...
    delBtn.addEventListener('click',async()=>{
      if(!confirm('هل تريد حذف حسابك نهائياً؟ لا يمكن التراجع!'))return;
      const pwd=prompt('أدخل كلمة مرورك للتأكيد:'); if(!pwd)return;
      const res=await Auth.deleteAccount(S.me,S.meEmail,pwd,S.groups);
      if(res.error)UI.toast(res.error,'error');
      else{S.me=null;S.meEmail=null;go('auth');}
  ...
  function renderUserProfile(){
    ...
    const mf=myFriends();
    if(mf.includes(u)&&S.stories[u]&&Object.keys(S.stories[u]).length>0){const stSec=UI.el('div',`margin-bottom:16px;padding:12px;background:${th().card};border-radius:14px;border:1px solid ${th().border}`);const stHdr=UI.el('div',`font-size:12px;color:${th().text2};font-weight:700;margin-bottom:10px`);stHdr.textContent='القصص';stSec.appendChild(stHdr);const stRow=UI.el('div','display:flex;gap:8px;overflow-x:auto;justify-content:center;scrollbar-width:none');Stories.getUserStories(u,S.stories).forEach(({data})=>{const img=document.createElement('img');img.src=data;img.loading='lazy';img.style.cssText='width:60px;height:80px;object-fit:cover;border-radius:8px;cursor:pointer;border:2px solid #6366f1;flex-shrink:0';img.addEventListener('click',()=>{Stories.markSeen(u);openStoryViewer(u);});stRow.appendChild(img);});stSec.appendChild(stRow);inner.appendChild(stSec);}
