...
(async function NabdaApp() {
  ...
  function startMsgListener(type, id) {
    ...
    const path = type==='dm' ? `dmMsgs/${Chat.dmKey(S.me, id)}` : `groupMsgs/${id}`;
  ...
  function startDataListeners() {
    addU(FB.on('users',          v=>{S.users=v||{}; rebuildUserMaps(); refreshDyn();}));
  ...
  function buildChatInput(type){
    ...
    const doSend=()=>{
      ...
      const tempMsg={sender:S.me,ts:Date.now(),id:tempId,...payload,_pending:true};
      _seenMsgIds.add(tempId);
      appendMsg({...tempMsg,_isOpt:true});
      _pendingMsgs.set(tempId, {payload, type, retries: 0});

      const sendFn = type==='dm'
        ? () => Chat.sendDM(S.me,S.activeId,payload)
        : () => Chat.sendGroup(S.activeId,S.me,payload);
