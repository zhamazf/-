...
(async function NabdaApp() {
  ...
  try {
    console.log('[BOOT] calling FB.init()...');
    await FB.init();
    console.log('[BOOT] FB.init() OK');
  } catch (err) {
    console.error('[BOOT] FB.init() FAILED:', err.message);
  ...
  function startDataListeners() {
    console.log('[DATA] startDataListeners — meUid:', S.meUid);
    addU(FB.onConnectionChange(isOnline => { console.log('[DATA] connection:', isOnline); UI.setConnectionStatus(isOnline); }));
    addU(FB.on('users', v => {
      console.log('[DATA] users snapshot — keys:', Object.keys(v||{}).length);
      S.users=v||{}; rebuildUserMaps(); refreshDyn(); _startDmListeners(); _startFriendReqListener(); _startStoryListeners(); _startGroupListeners();
    }));
  ...
  function renderAuth() {
    ...
    async function doSubmit() {
      ...
      console.log('[AUTH] doSubmit mode:', mode, 'email:', emailInp.value);
      let res;
      if (mode==='login') {
      ...
      console.log('[AUTH] success — me:', S.me, 'meUid:', S.meUid);
  ...
  if (S.me) {
    console.log('[BOOT] S.me:', S.me, '— restoring session');
    S.meUid = FB.authCurrentUser()?.uid || null;
    console.log('[BOOT] S.meUid:', S.meUid);
    Chat.goOnline(S.meUid);
    UI.requestNotifPerm();
    startDataListeners();
    const checkStart = Date.now();
    const checkData = () => {
      console.log('[BOOT] checkData — users:', Object.keys(S.users).length, 'groups:', Object.keys(S.groups||{}).length, 'elapsed:', Date.now()-checkStart);
      if (Object.keys(S.users).length > 0 || Object.keys(S.groups || {}).length > 0) {
        console.log('[BOOT] data loaded → showApp');
        UI.showApp();
        go('home');
      } else if (Date.now() - checkStart > 8000) {
        console.warn('[BOOT] timeout 8s — showing app anyway');
        UI.showApp();
        go('home');
      } else {
    ...
    console.log('[BOOT] no session → go auth');
    UI.showApp();
    go('auth');
  }
