...
(async function NabdaApp() {
  ...
  if (S.me) {
    ...
    // إذا لم يكن هناك uid (جلسة قديمة بدون Firebase Auth) → روح للـ auth
    if (!S.meUid) {
      console.warn('[BOOT] no meUid — clearing session');
      Auth.clearSession();
      UI.showApp();
      go('auth');
    } else {
      ...
      UI.requestNotifPerm();
      startDataListeners();
      // افتح التطبيق مباشرة بعد ثانية واحدة بدون انتظار البيانات
      setTimeout(() => {
        console.log('[BOOT] showApp after 1s');
        UI.showApp();
        go('home');
      }, 1000);
    }
