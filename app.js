...
(async function NabdaApp() {
  ...
  function startMsgListener(type, id) {
    ...
    // dmKey must be uid-based for security rules ($dmKey.contains(auth.uid))
  ...
  function startDataListeners() {
    addU(FB.on('users',    v=>{S.users=v||{}; rebuildUserMaps(); refreshDyn(); _startDmListeners(); _startFriendReqListener();}));
    ...
    // Defer stories loading by 2s to prioritise core data
    setTimeout(() => {
      addU(FB.on('stories', v=>{
        S.stories=v||{};
        Stories.cleanExpired(v||{});
        refreshDyn();
      }));
      // Periodic expired-story cleanup every 5 minutes
      if (!_storiesTimer) _storiesTimer = setInterval(()=>Stories.cleanExpired(S.stories), 300000);
    }, 2000);
  }

  // ── DM listeners — one per friend uid pair (Rules allow only specific dmKey paths) ──
  const _dmUnsubs = {};
  function _startDmListeners() {
    if (!S.meUid) return;
    const myUid = S.meUid;
    const friendUids = Object.keys(S.users[myUid]?.friends || {});
    friendUids.forEach(fUid => {
      // dmKey must be uid-based to match security rules: $dmKey.contains(auth.uid)
      const key = [myUid, fUid].sort().join('_');
  ...
  function buildChatInput(type){
    ...
    const doSend=()=>{
      ...
      const tempMsg={sender:S.meUid,ts:Date.now(),id:tempId,...payload,_pending:true};
  ...
  function buildGroupPanel(parentEl){
    ...
    panel.appendChild(memSec); parentEl.appendChild(panel);
  }
